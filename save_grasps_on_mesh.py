...
if __name__ == "__main__":
    ...
    #     'run_history',
    #     '2023-05-04-18-09-51')
    run_history_dir = os.path.join('/media/sungwon/WorkSpace/projects/isaacgym-gqcnn-results/2023-05-14-23-34-10')
    ...
    for episode, episode_dir in enumerate(episode_dirs):
        ...
        for i in range(100):
            ...
            if isinstance(action.grasp, Grasp2D):
                ...
                # append data
                sc_mesh_grasp_dataset[target_object_idx].actions.append(action)
                sc_mesh_grasp_dataset[target_object_idx].successes.append(reward)
                sc_mesh_grasp_dataset[target_object_idx].grasp_poses.append(grasp_pose_o)
    ...
    for mesh_grasp_data in sc_mesh_grasp_dataset:
        assert isinstance(mesh_grasp_data, MeshGraspData)
        if len(mesh_grasp_data.actions) == 0:
            print('No actions on {}'.format(mesh_grasp_data.mesh_name))
            continue
        sc_priors = sc_grasp_planner.policy.evaluate_action(mesh_grasp_data.actions)
        mesh_grasp_data.priors = sc_priors
    ...
    for mesh_grasp_data in sc_mesh_grasp_dataset:
        ...
        if len(mesh_grasp_data.actions) == 0:
            ...
            continue
        for action, reward in zip(mesh_grasp_data.actions, mesh_grasp_data.successes):
            sc_grasp_planner.policy.update(action, reward)
    ...
    for mesh_grasp_data in sc_mesh_grasp_dataset:
        ...
        if len(mesh_grasp_data.actions) == 0:
            print('No actions on {}'.format(mesh_grasp_data.mesh_name))
            continue
        sc_posteriors = sc_grasp_planner.policy.evaluate_action(mesh_grasp_data.actions)
        mesh_grasp_data.posteriors = sc_posteriors
    ...
    for mesh_name in obejct_names:
        ...
        os.makedirs(os.path.join(sc_save_dir, mesh_name), exist_ok=True)
        sc_mesh_grasp_data = sc_mesh_grasp_dataset[obejct_names.index(mesh_name)]
        pickle.dump(sc_mesh_grasp_data.grasp_poses, open(os.path.join(sc_save_dir, mesh_name, 'grasp_poses.pkl'), 'wb'))
        pickle.dump(sc_mesh_grasp_data.successes, open(os.path.join(sc_save_dir, mesh_name, 'successes.pkl'), 'wb'))
        pickle.dump(sc_mesh_grasp_data.priors, open(os.path.join(sc_save_dir, mesh_name, 'priors.pkl'), 'wb'))
        pickle.dump(sc_mesh_grasp_data.posteriors, open(os.path.join(sc_save_dir, mesh_name, 'posteriors.pkl'), 'wb'))
